...
Задай один короткий вопрос чтобы дополнить недостающее.`;
  ...
  return llmChat(ROUNDTRIP_SYSTEM, userPrompt, { task: 'extract', temperature: 0.5, maxTokens: 200 });
}
...
Верни обновлённые 5 полей в JSON.`;
  ...
  const raw = await llmChat(MERGE_SYSTEM, userPrompt, { task: 'extract', temperature: 0.2 });
